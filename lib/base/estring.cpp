...
std::string ConvertUnicode2UTF8(const unsigned char *data, int len) //input BIGEND Unicode
{
	...
	while (i < len)
	{
		if (i+1 < len) { //			code=((data[i] << 8) & 0xFF00) | (data[i+1] & 0xFF);
...
//			eDebug("[convertDVBUTF8] (0x10)text encoded in ISO-8859-%d",n);
		...
		case 0x13:
			...
			FORCE_GBK = true;
			len = ConvertGBK2Unicode(UTF, len-1, (char*)data+1);
			FORCE_GBK = false;
			return ConvertUnicode2UTF8(UTF,len);		
		case 0x14:
			++i;
			eDebug("Big5 subset of ISO/IEC 10646-1 enc.");
			return ConvertUnicode2UTF8((char*)data+1,len-1);
...
int ConvertBig52Unicode(unsigned char* dest, int src_size, unsigned char* src) {
	...
	for (i = 0, j = 0; j < src_size; i += 2) {
		if (i+2 > 2046)
		{
			eDebug("ConvertBig52Unicode buffer to small.. break now");
			break;
		}
		h = src[j++] & 0xff; //读取一个字节 DB,BF
		if (h == 0x00) break;
		if ((h & 0x80) == 0) { // 高位为0 是Ascii 码
			l = 0x0;
		} else {
			l = src[j++] & 0xff; // 高位为1 为gbk字符集字符,读取下一个字节 DA,A8
			if ( l > 0x7E)
				seek = (h - 0x81) * 157 + (l - 0x40 -0x22);
			else
				seek = (h - 0x81) * 157 + (l - 0x40);
			seek = 2 + seek * 2;
			h = TABLE_BIG5[seek];
			l = TABLE_BIG5[seek + 1];
		}
...
int ConvertGBK2Unicode(unsigned char* dest, int src_size, unsigned char* src) {
	...
	for (i = 0, j = 0; j < src_size; i += 2) {
		if (i+2 > 2046)
		{
			eDebug("ConvertGBK2Unicode buffer to small.. break now");
			break;
		}
		h = src[j++] & 0xff; //读取一个字节
		if (h == 0x00) break;
		if ((h & 0x80) == 0) { // 高位为0 是Ascii 码
			...
			//将gbk编码映射到ULE表中的位置
			//GBK编码区域
			//GBK1 A1A1-A9FE 717/846
			//GBK2 B0A1-F7FE 6763/6768
			//GBK3 8140-A0FE 6080/6080 低位无7F
			//GBK4 AA40-FEA0 8160/8160 低位无7F
			//GBK5 A840-A9A0 166/192 低位无7F
			//有效字符 21886/23940
			//BIG5编码区域
			...
			//繁体中文. 低位有两个区间 0x40-0x7E， 0xA1-0xFE，后一区间与GBK重复，忽略
			if (!FORCE_GBK && (( h >= 0xA1 && h <= 0xA3 && l >= 0x40 && l <= 0x7E) || ( h >= 0xA4 && h <= 0xC6 && l >= 0x40 && l <= 0x7E) ) ){
				eDebug("[vzDebug] is Big5");
				isbig5 = true;
				break; //如果是繁体中文则跳过GBK处理
			} else if (h >= 0xB0 && h <= 0xF7 && l >= 0xA1 && l <= 0xFE) { //GBK/2的编码范围 ,实际上GBK2高字节包含在GBK/4的范围内,只是低字节的范围不同
				...
				seek = GBK3_OFFSET + ((h - 0x81) * 190 + (l - 0x40)
						- (l > 0x7F ? 1 : 0)) * 2;//GBK3偏移+区位偏移+区内码数偏移 因为低位字节0x7F处不编码,所以大于0x7F要-1已更正位置. 190=0xFE - 0x40+1-1 低字节 0x0-0x39 与 0x7F,0xFF 不放  总共空出66个字节
				h = TABLE_GBK[seek];
				l = TABLE_GBK[seek + 1];
		...
		dest[i] = l;
		dest[i + 1] = h;
//		eDebug("[+l] ....%#02x ", (unsigned short)l);
//		eDebug("[+h] ....%#02x ", (unsigned short)h);
	}
	if (!isbig5) {
		dest[i] = '\0';	//wchar 宽字符字符串的结束标记
		dest[i+1] = '\0';
		return i;
	}
	else
...
std::string getOSDcountryCode(){
	const char MAX_LANG = 9;
	std::string ISO639tbl[MAX_LANG][2] =
	{
		{"C","eng"},
		{"zh_CN","chi"},
		{"hk_HK","chi"},
	...
	std::string OSDcountryCode = eConfigManager::getConfigValue("config.osd.language");
	if (OSDcountryCode != "")
		for (int i=0; i < MAX_LANG; i++)
			if (OSDcountryCode==ISO639tbl[i][0]){
				OSDcountryCode = ISO639tbl[i][1];
				break;
			}
	return OSDcountryCode;
}
